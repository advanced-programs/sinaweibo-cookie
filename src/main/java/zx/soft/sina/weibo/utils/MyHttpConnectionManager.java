...
import zx.soft.sina.weibo.common.ReadSpiderConfig;
...
public class MyHttpConnectionManager {
	...
	// 数据设置
	// 最大链接数
	public static int max_connection = Integer.parseInt(ReadSpiderConfig.getValue("max_connections"));

	// 获取链接的最大等待时间
	public static int wait_connection_timeout = Integer.parseInt(ReadSpiderConfig.getValue("wait_connection_timeout"));

	// 连接超时时间
	public static int connection_timeout = Integer.parseInt(ReadSpiderConfig.getValue("connection_timeout"));

	// 读取超时
	public static int read_timeout = Integer.parseInt(ReadSpiderConfig.getValue("read_timeout"));
	...
	public static void setHandleRedirect(DefaultHttpClient defaultClient, boolean isAuto) {
		if (isAuto) {
			defaultClient.getParams().setParameter(ClientPNames.HANDLE_REDIRECTS, true);
		} else {
			defaultClient.getParams().setParameter(ClientPNames.HANDLE_REDIRECTS, false);
		}
	}
